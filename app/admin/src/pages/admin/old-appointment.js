...
const Index = () => {
    ...
    let home = false, icon = 'arrow', items = [
        ...
        { title: 'Old Appoinment', url: '/admin/old-appointment', last: true }
    ];

    let get_appointment = (tabops) => {
        let response = fetchData('/admin/appointments/old_appointment', 'post', tabops);
        response.then((res) => {
    ...
    let get_edit_data = (e, _id) => {
        ...
        if (_id && _id.length > 0 && String(_id) !== String(undefined)) {
            setselected_id(_id);
            let response = fetchData('/admin/appointments/edit', 'post', { _id });
            response.then((res) => {
                if (res && res.data && String(res.data.status) === '00') {
                    return router.push('/admin/login')
                }
    ...
    let DeleteJobs = (e) => {
        ...
        if (deleted_id && deleted_id.length > 0 && String(deleted_id) !== String(undefined)) {
            let response = fetchData('/admin/appointments/soft_delete', 'post', { _id: deleted_id });
            response.then((res) => {
                if (res && res.data && +res.data.status === 0) {
                    if (res.data.errors && res.data.errors.length > 0 && Array.isArray(res.data.errors)) {
                        res.data.errors.map((item) => {
    ...
    return (
        <>
            ...
            <SectionTitle title="" subtitle='Appointment' />
            <Widget title="">
                        ...
                        <tr>
                            ...
                            <th>
                                <span style={{ display: 'inline-flex' }}>
                                    Date
                            {
                                        tableOptions.field === 'appointment_date' ?
                                            +tableOptions.order === 1 ? <FaSortNumericUp style={{ margin: '0 5px', cursor: 'pointer' }} onClick={e => sort_created(e, 'appointment_date', -1)} />
                                                : <FaSortNumericDown style={{ margin: '0 5px', cursor: 'pointer' }} onClick={e => sort_created(e, 'appointment_date', 1)} /> :
                                            <FaSortNumericDown style={{ margin: '0 5px', cursor: 'pointer' }} onClick={e => sort_created(e, 'appointment_date', 1)} />
                                    }
                                </span>
                            </th>
                            ...
                            <th>   <span style={{ display: 'inline-flex' }}>
                                Gender
                            {
                                    tableOptions.field === 'gender' ?
                                        +tableOptions.order === 1 ? <FaSortNumericUp style={{ margin: '0 5px', cursor: 'pointer' }} onClick={e => sort_created(e, 'gender', -1)} />
                                            : <FaSortNumericDown style={{ margin: '0 5px', cursor: 'pointer' }} onClick={e => sort_created(e, 'gender', 1)} /> :
                                        <FaSortNumericDown style={{ margin: '0 5px', cursor: 'pointer' }} onClick={e => sort_created(e, 'gender', 1)} />
                                }
                            </span></th>
                            <th>   <span style={{ display: 'inline-flex' }}>
                                Age
                            {
                                    tableOptions.field === 'age' ?
                                        +tableOptions.order === 1 ? <FaSortNumericUp style={{ margin: '0 5px', cursor: 'pointer' }} onClick={e => sort_created(e, 'age', -1)} />
                                            : <FaSortNumericDown style={{ margin: '0 5px', cursor: 'pointer' }} onClick={e => sort_created(e, 'age', 1)} /> :
                                        <FaSortNumericDown style={{ margin: '0 5px', cursor: 'pointer' }} onClick={e => sort_created(e, 'age', 1)} />
                                }
                            </span></th>
                            <th>   <span style={{ display: 'inline-flex' }}>
                                Department
                            {
                                    tableOptions.field === 'department_name' ?
                                        +tableOptions.order === 1 ? <FaSortNumericUp style={{ margin: '0 5px', cursor: 'pointer' }} onClick={e => sort_created(e, 'department_name', -1)} />
                                            : <FaSortNumericDown style={{ margin: '0 5px', cursor: 'pointer' }} onClick={e => sort_created(e, 'department_name', 1)} /> :
                                        <FaSortNumericDown style={{ margin: '0 5px', cursor: 'pointer' }} onClick={e => sort_created(e, 'department_name', 1)} />
                                }
                            </span></th>
                            <th>Actions</th>
                            ...
                            table_arr.map((item, i) => {
                                return (
                                    <tr key={i}>
                                        ...
                                        <td>{item.name}</td>
                                        <td>{item.email}</td>
                                        <td>{item.gender ? String(item.gender).toUpperCase() : ''}</td>
                                        <td>{item.age}</td>
                                ...
                                <div className="relative min-w-sm w-auto mx-auto lg:max-w-5xl">
                                    <div className="modal-content">
                                        <div className="modal-header">
                                            <h3 className="text-xl font-semibold">{'View Patient'}</h3>
                                            <button
                                        ...
                                        <div className="relative p-4 flex-auto" style={{ width: '600px' }}>
                                            <div className={`form-element`}>
                                                ...
                                                <input
                                                        ...
                                                        style={{ float: 'right' }}
                                                        disabled={true}
                                                        value={view_obj && view_obj.gender ? String(view_obj.gender).toUpperCase() : ''}
                                                    />
                                                </div>
                                                <div className="form-label">Age :
                                                <input
                                                        className={`form-control`}
                                                        style={{ float: 'right' }}
                                                        disabled={true}
                                                        value={view_obj && view_obj.age ? view_obj.age : ''}
                                                    />
                                                </div>
                                                <div className="form-label">SMobile :
                                                <input
                                                        className={`form-control`}
                                                        style={{ float: 'right' }}
                                                        disabled={true}
                                                ...
                                                <div className="form-label">Department :
                                                <input
                                                        className={`form-control`}
                                                        disabled={true}
                                                        style={{ float: 'right' }}
                                                        value={view_obj && +view_obj.department_name ? view_obj.department_name : ''}
                                                    />
                                                </div>
                                                ...
                                                <input
                                                        ...
                                                        disabled={true}
                                                        style={{ float: 'right' }}
                                                        value={view_obj && +view_obj.appointment_date ? moment(new Date(view_obj.appointment_date)).format('DD-MM-YYYY') : ''}
                                                    />
                                                </div>
                                                ...
                                                <input
                                                        ...
                                                        disabled={true}
                                                        style={{ float: 'right' }}
                                                        value={view_obj && +view_obj.time ? view_obj.time : ''}
                                                    />
                                                </div>
                                            </div>
                                        </div>
