...
import { FaSortNumericDown, FaSortNumericUp, FaRegFilePdf } from 'react-icons/fa'
...
const Index = () => {
    ...
    let home = false, icon = 'arrow', items = [
        ...
        { title: 'Manage Resume', url: '/admin/manage-resume', last: true }
    ];

    let get_applied_jobs = (tabops) => {
        let response = fetchData('/admin/applied_jobs/list', 'post', tabops);
        response.then((res) => {
    ...
    let get_edit_data = (e, _id) => {
        ...
        if (_id && _id.length > 0 && String(_id) !== String(undefined)) {
            setselected_id(_id);
            let response = fetchData('/admin/applied_jobs/edit', 'post', { _id });
            response.then((res) => {
                if (res && res.data && String(res.data.status) === '00') {
                    return router.push('/admin/login')
                }
    ...
    let DeleteJobs = (e) => {
        ...
        if (deleted_id && deleted_id.length > 0 && String(deleted_id) !== String(undefined)) {
            let response = fetchData('/admin/applied_jobs/soft_delete', 'post', { _id: deleted_id });
            response.then((res) => {
                if (res && res.data && +res.data.status === 0) {
                    if (res.data.errors && res.data.errors.length > 0 && Array.isArray(res.data.errors)) {
                        res.data.errors.map((item) => {
    ...
    return (
        <>
            ...
            <SectionTitle title="" subtitle='Manage Resume' />
            <Widget title="">
                        ...
                        <tr>
                            ...
                            <th>
                                <span style={{ display: 'inline-flex' }}>
                                    Job Title
                            {
                                        tableOptions.field === 'cat_name' ?
                                            +tableOptions.order === 1 ? <FaSortNumericUp style={{ margin: '0 5px', cursor: 'pointer' }} onClick={e => sort_created(e, 'cat_name', -1)} />
                                                : <FaSortNumericDown style={{ margin: '0 5px', cursor: 'pointer' }} onClick={e => sort_created(e, 'cat_name', 1)} /> :
                                            <FaSortNumericDown style={{ margin: '0 5px', cursor: 'pointer' }} onClick={e => sort_created(e, 'cat_name', 1)} />
                                    }
                                </span>
                            </th>
                            ...
                            table_arr.map((item, i) => {
                                return (
                                    <tr key={i}>
                                        ...
                                        <td>{item.cat_name ? item.cat_name : item.jobs_data && item.jobs_data.name ? item.jobs_data.name : ''}</td>
                                        <td>{item.name}</td>
                                        <td>{item.email}</td>
                                        <td>{item.docs ? <FaRegFilePdf size={20} style={{ cursor: 'pointer' }} onClick={e => clickdocs(e, item.docs)} /> : null}</td>
                                        <td>
                                    ...
                                    <div className="modal-content">
                                        ...
                                        <div className="relative p-4 flex-auto" style={{ width: '600px' }}>
                                            <div className={`form-element`}>
                                                <div className="form-label">Job Title :
                                                <input
                                                        className={`form-control`}
                                                        style={{ float: 'right' }}
                                                        disabled={true}
                                                        value={view_obj && view_obj.cat_name ? view_obj.cat_name : view_obj.jobs_data && view_obj.jobs_data.name ? view_obj.jobs_data.name : ''}
                                                    />
                                                </div>
