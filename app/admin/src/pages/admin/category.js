...
const Index = () => {
    ...
    let home = false, icon = 'arrow', items = [
        ...
        { title: 'Category', url: '/admin/category', last: true }
    ];

    let get_category = (tabops) => {
        let response = fetchData('/admin/category/list', 'post', tabops);
        response.then((res) => {
    ...
    let submit = (e) => {
        ...
        let response = fetchData('/admin/category/add', 'post', data);
        response.then((res) => {
            setclicked(false);
            if (res && res.data && String(res.data.status) === '00') {
                return router.push('/admin/login')
    ...
    let get_edit_data = (e, _id, type) => {
        ...
        if (_id && _id.length > 0 && String(_id) !== String(undefined)) {
            setselected_id(_id);
            let response = fetchData('/admin/category/edit', 'post', { _id });
            response.then((res) => {
                if (res && res.data && String(res.data.status) === '00') {
                    return router.push('/admin/login')
                }
    ...
    let DeleteJobs = (e) => {
        ...
        if (deleted_id && deleted_id.length > 0 && String(deleted_id) !== String(undefined)) {
            let response = fetchData('/admin/category/soft_delete', 'post', { _id: deleted_id });
            response.then((res) => {
                if (res && res.data && +res.data.status === 0) {
                    if (res.data.errors && res.data.errors.length > 0 && Array.isArray(res.data.errors)) {
                        res.data.errors.map((item) => {
    ...
    return (
        <>
            ...
            <SectionTitle title="" subtitle={!view_show && selected_id && selected_id.length > 0 && String(selected_id) !== String(undefined) ? "Edit category" : "Add category"} />
            <Widget title="">
                <form>
                    <div className="flex flex-col lg:flex-row lg:flex-wrap w-full lg:w-1/2">
                        <div className="w-full mt-3 mb-6">
                            <div className={`form-element`}>
                                ...
                                />
                                {name_err ? <div className="form-error">Category name is required</div> : null}
                            </div>
                        </div>
                        <div className="w-full mt-3 mb-6">
                            <div className={`form-element`}>
                                ...
                                <div className="relative min-w-sm w-auto mx-auto lg:max-w-5xl">
                                    <div className="modal-content">
                                        <div className="modal-header">
                                            <h3 className="text-xl font-semibold">{'View category'}</h3>
                                            <button
