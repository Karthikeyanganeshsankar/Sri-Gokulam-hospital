...
var io = require("socket.io")(server); // Allow Cross Domain Requests
...
mongoose.connection.on("error", function (error) {
  return console.error("Error in MongoDb connection: " + error);
});
mongoose.connection.on("reconnected", function () {
  return console.log("Trying to reconnect!");
});
...
mongoose.connection.on("connected", function () {
  ...
  app.use(bodyParser.json({
    limit: "100mb"
  })); // bodyParser - Initializing/Configuration
  ...
  app.locals.pretty = true; // app.use('/portal', express.static(path.join(__dirname, '/public/portal'), { maxAge: 7 * 86400000 }));
